...
#-----------------------------------
class QwenMaxChatWarpperClient(ModelClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        LOG_INFO("QwenMaxChatWarpperClient::__init__-----------")
    ...
    def format(self, *args: Union[Msg, Sequence[Msg]]) -> List[dict]:
        ...
        #---------------------------------------------------
        for arg in args:
            LOG_INFO("QwenMaxChatWarpperClient::format=============== arg in args:",arg)
            ...
            if isinstance(arg, Msg): # 如果是 Msg
                    ...
                    if arg.role != "tool":
                        ...
                        if arg.get("message"):
                            ...
                            if function_call_msg.get("raw"):
                                LOG_INFO("QwenMaxChatWarpperClient::format:=============== message in Msg:",arg.get("message"))
                        ...
                        # 下面是 function call 调用后的结果的消息
                         #---------------------------------------------------
                            ...
                            LOG_INFO("QwenMaxChatWarpperClient::format:=============== tool_responses in Msg:",arg.get("tool_responses"))
                ...
                LOG_INFO("QwenMaxChatWarpperClient::format=============== arg to message:",messages)
        ...
        LOG_INFO("QwenMaxChatWarpperClient::format=============== end! final messages:",messages)
    ...
    def format_function_call_rsp(self, responses:List[dict]) -> List[dict]:
        ...
        message = {
            "raw":responses,# 保存原始的响应，原始消息包含所有内容
            "tool_calls": tool_calls
        }
        # raw 里面存的是是 qwen原始是响应格式。后续需要用到
        formatted_response =Msg(name="assistant",role="tool",content = "", message = message)
        LOG_INFO("QwenMaxChatWarpperClient::format_function_call_rsp:----------- formatted_response:\n",formatted_response)
    ...
    def _generate_response(
        ...
        LOG_INFO("QwenMaxChatWarpperClient::_generate_response:----------- kwargs",kwargs)
        ...
        # 3： 调用模型接口 同步
        #-------------------------------------------------------------
        LOG_INFO("QwenMaxChatWarpperClient::_generate_response:------cccc----- qwen_call_args:\n",qwen_call_args)
        #response = self.client.chat.completions.create(**kwargs)
        responses = self.client.chat(**qwen_call_args)
        # 千万返回的是一个list，里面可能包含了多个消息
        LOG_INFO("QwenMaxChatWarpperClient::_generate_response:------cccc----- response:\n",responses)
        ...
        if stream:
            ...
            LOG_INFO("QwenMaxChatWarpperClient::_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
            #=======================================================
            ...
            if self._verify_function_call_in_qwen_stream_message_response(first_chunk[0]):
                ...
                for response in responses:
                    #LOG_INFO("QwenMaxChatWarpperClient::_generate_response:-------dddd-1-stream--response:\n",response)
                    pass

                LOG_INFO("QwenMaxChatWarpperClient::_generate_response:-------dddd-1-stream--finish response:\n",response)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO("QwenMaxChatWarpperClient::_generate_response:generator--------------->last_chunk:\n",chunk,flush=True)
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
            ...
            # 如果响应只有一个内容，则按照qwen2的方式处理。
            if len(responses) == 1:
                LOG_INFO("QwenMaxChatWarpperClient::_generate_response:-------ccc222---- responses[0]:\n",responses[0])

                #判断当前响应是不是给用户的合法响应,这个响应只有一个内容
                if self._verify_text_content_in_qwen_message_response(responses[0]):
                    LOG_INFO("QwenMaxChatWarpperClient::_generate_response:----------- rsp:",responses[0])
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
    ...
    async def _a_generate_response(
        ...
        # 其他参数
        kwargs = {**self.generate_args, **kwargs}
        LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:----------- kwargs",kwargs)
        ...
        # 3： 调用模型接口 异步
        #-------------------------------------------------------------
        LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:------cccc----- qwen_call_args:\n",qwen_call_args)
        responses = self.client.chat(**qwen_call_args)
        LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:------cccc----- response:\n",responses)
        ...
        if stream:
            ...
            # 第一个消息即可判断是 function call 还是普通消息
            first_chunk = next(responses)
            LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
            #=======================================================
            #    如果是 流式开关开启，根据迭代器第一个消息判断是function call 还是普通消息  
            #   ***** 如果是function call的流式消息
            #=======================================================
            if self._verify_function_call_in_qwen_stream_message_response(first_chunk[0]):
                # 遍历迭代生成器， qwen的流式返回的内容是增量的, 所以直接获取最后一个即可
                for response in responses:
                    #LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:-------dddd-1-stream--response:\n",response)
                    pass

                LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:-------dddd-1-stream--finish response:\n",response)
                abc = self.format_function_call_rsp(response)
                #---------------------------------------------------
                #  tool call调用的消息，保存为历史消息，
                #       后续需要调用大模型llm的时候携带这个和调用结果上去
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    #print(f"------------5555555555------------last_chunk: {text}", flush=True)  # 增加这一行
                    LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:generator--------------->last_chunk:\n",chunk,flush=True)
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:-------ccc222---- responses[0]:\n",responses[0])

            #判断当前响应是不是给用户的合法响应,这个响应只有一个内容
            if self._verify_text_content_in_qwen_message_response(responses[0]):
                LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:----------- rsp:",responses[0])
                #---------------------------------------------------
                #  调用回调函数，保存历史消息
                #---------------------------------------------------
                if callback is not None:
            ...
            # 判断是否合法的 function call, 如果返回的是一个空的content，但是有function call的时候，这是一个function call消息
                ...
                # 将qwen格式的响应转换成标准的openai格式
                LOG_INFO("QwenMaxChatWarpperClient::_a_generate_response:----------- new format response:",responses) 
