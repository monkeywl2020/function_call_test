...
class OpenAiChatWarpperClient(ModelClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        LOG_INFO("OpenAiChatWarpperClient::format:=============== arg in self.stream:",self.stream)
    ...
    def _generate_response(
        ...
        if stream:
            ...
            try:
                ...
                LOG_INFO("OpenAiChatWarpperClient::_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("OpenAiChatWarpperClient::_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("OpenAiChatWarpperClient::_generate_response: -------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("OpenAiChatWarpperClient::_generate_response: -------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("OpenAiChatWarpperClient::_generate_response: -------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"OpenAiChatWarpperClient::_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
    ...
    async def _a_generate_response(
        ...
        # 3： 调用模型接口 异步
        #-------------------------------------------------------------
        LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------cccc---- openAikwargs:",openAikwargs)
        response = self.client.chat.completions.create(**openAikwargs)
