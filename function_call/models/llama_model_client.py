...
class LlamaChatWarpperClient(ModelClient):
    ...
    def _generate_response(
        ...
        if stream:
            llamakwargs["stream_options"] = {"include_usage": True}

        # 如果标识使用工具，并且有工具管理器,则获取工具描述
        if use_tools and self.tools_manager:
            tools = self.tools_manager.get_tool_descriptions(type = "openai")
            if len(tools) > 0:
                llamakwargs["tools"] = tools

        #-------------------------------------------------------------
        # 3： 调用模型接口 同步
        #-------------------------------------------------------------
        LOG_INFO("LlamaChatWarpperClient::_generate_response:-------cccc---- llamakwargs:\n",llamakwargs)
        response = self.client.chat.completions.create(**llamakwargs)
    ...
    def _verify_function_call_in_llama_message_response(self,response: dict) -> bool:
        ...
        if len(response.get("choices", [])) == 0:
            LOG_INFO("=_verify_function_call_in_llama_message_response========1======False")
            return False

        if response["choices"][0].get("message", None) is None:
            LOG_INFO("=_verify_function_call_in_llama_message_response========2======False")
            return False
        
        if response["choices"][0]["message"].get("tool_calls", None) is None:
            LOG_INFO("=_verify_function_call_in_llama_message_response========3======False")
            return False
