...
#-----------------------------------
class WenXinChatWarpperClient(ModelClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        LOG_INFO("WenXinChatWarpperClient::__init__-----------")
        ...
        try:
            ...
            raise ImportError(
                "Cannot find qianfan package, please install it by "
                "`pip install qianfan`",
            ) from e
        ...
        os.environ["QIANFAN_SK"] = kwargs['secret_key']
    ...
    def format(self, *args: Union[Msg, Sequence[Msg]]) -> List[dict]:
        ...
        #---------------------------------------------------
        for arg in args:
            LOG_INFO("WenXinChatWarpperClient::format:=============== arg in args:",arg)
            ...
            if isinstance(arg, Msg): # 如果是 Msg
                if arg.url is not None:# 如果有 url
                    ...
                    if arg.role != "tool":
                        ...
                        #---------------------------------------------------
                        if arg.get("message"):
                            LOG_INFO("WenXinChatWarpperClient::format:=============== message in Msg:",arg.get("message"))
                            raw_msg = arg.get("message").get("raw")
                            wenxin_function_call_rsp = {
                                "role": "assistant",
                                "content": "",
                                "function_call": raw_msg,
                            }
                            messages.append(wenxin_function_call_rsp)

                        # 下面是工具调用结果的消息
                        elif arg.get("tool_responses"):
                            LOG_INFO("WenXinChatWarpperClient::format:=============== tool_responses in Msg:",arg.get("tool_responses"))
                            for tool_response in arg.get("tool_responses"):
                                wenxin_function_call_reply ={
                                    "role": "function",
                                    "name": tool_response.get("name"),
                                    "content": tool_response.get("content"),
                                }
                                messages.append(wenxin_function_call_reply)
                            #messages.append()

                        else:
                            messages.append(arg)
                LOG_INFO("WenXinChatWarpperClient::format:=============== arg to message:",messages)
        ...
        LOG_INFO("WenXinChatWarpperClient::format:=============== end! final messages:",messages)
        return messages
    ...
    def format_function_call_rsp(self, response:dict) -> List[dict]:
        ...
        'function_call': {
            ...
            'thoughts': '用户想要知道武汉、北京和深圳的当前天气情况。根据用户需求，我需要首先调用获取天气信息的工具，然后分别查询武汉、北京和深圳的天气。任务拆解：[sub-task1: 调用[get_current_weather]工具查询武汉的天气，sub-task2: 调用[get_current_weather]工具查询北京的天气，sub-task3: 调用[get_current_weather]工具查询深圳的天气]。接下来需要调用[get_current_weather]工具来查询武汉的天气情况。',
        ...
        #  raw是用来做 function call的reply处理的时候用的。 tool_calls是用来做tools调用的时候用的
        message = {
            "raw":func_call,
        ...
        LOG_INFO("WenXinChatWarpperClient::format_function_call_rsp:----------- formatted_response:\n",formatted_response)
    ...
    def _generate_response(
        ...
        LOG_INFO("WenXinChatWarpperClient::_generate_response:---------11-- kwargs",kwargs)
        ...
        if stream:
            WenXinkwargs["stream_options"] = {"include_usage": True}

        # 如果标识使用工具，并且有工具管理器,则获取工具描述, wenxin的和openAI的function描述类似
        if use_tools and self.tools_manager:
            tools = self.tools_manager.get_tool_descriptions(type = "wenxin")
            if len(tools) > 0:
                WenXinkwargs["functions"] = tools

        #-------------------------------------------------------------
        # 3： 调用模型接口 同步
        #-------------------------------------------------------------
        LOG_INFO("WenXinChatWarpperClient::_generate_response:-------cccc---- kwargs:\n",WenXinkwargs)
        response = self.client.do(**WenXinkwargs)
        LOG_INFO("WenXinChatWarpperClient::_generate_response:-------cccc---- response:\n",response)
        ...
        if stream:
            ...
            try:
                ...
                LOG_INFO("WenXinChatWarpperClient::_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("WenXinChatWarpperClient::_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("WenXinChatWarpperClient::_generate_response: -------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("WenXinChatWarpperClient::_generate_response: -------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("WenXinChatWarpperClient::_generate_response: -------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"WenXinChatWarpperClient::_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            if self._verify_function_call_in_wenxin_message_response(response):
                func_response = response.get("function_call")
                LOG_INFO("WenXinChatWarpperClient::_generate_response:-------cccc222-1---- response:\n",func_response)
                wenxin_rsp = self.format_function_call_rsp(func_response)
                ...
                #---------------------------------------------------
                if callback is not None:
                    abc = Msg(name="assistant",role="tool", content = "", message = func_response)
                    callback(abc) #回调函数

                # 返回 response，放到 raw中,供后续使用， text取 choices[0]中的内容
                return ModelResponse(
                    text= wenxin_rsp["message"],
                    raw=response,
                    is_funcall_rsp = True,
                )
                
            elif self._verify_text_content_in_wenxin_message_response(response):
                func_response_body = response.get("body")
                LOG_INFO("WenXinChatWarpperClient::_generate_response:-------cccc222-2---- response:\n",func_response_body)
                #---------------------------------------------------
                #  如果是普通响应, 调用回调函数，保存历史消息
                #---------------------------------------------------
                if callback is not None:
                    abc = Msg(name="assistant",role="assistant",content=func_response_body.get("result"))
                    callback(abc) #回调函数

                # return response
                return ModelResponse(
                    text=func_response_body.get("result"),
                    raw=response,
                )
    ...
    async def _a_generate_response(
        ...
        LOG_INFO("WenXinChatWarpperClient::_a_generate_response-----------")
        # 其他参数
        kwargs = {**self.generate_args, **kwargs}
        ...
        # 3： 调用模型接口 异步
        #-------------------------------------------------------------
        LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------cccc---- kwargs:",kwargs)
        response = self.client.chat.completions.create(**kwargs)
        LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------cccc---- response:",response)
        ...
        if stream:
            ...
            try:
                ...
                first_chunk = first_chunk.model_dump()
                firs_role = first_chunk["choices"][0]["delta"]["role"]
                LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            except StopIteration:
                raise RuntimeError("Empty response received from OpenAI API.")
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"WenXinChatWarpperClient::_a_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            LOG_INFO("WenXinChatWarpperClient::_a_generate_response:-------cccc222---- response:model_dump():",response)
    ...
    #=======================================================
    def _verify_function_call_in_wenxin_message_response(self,response: dict) -> bool:

        if len(response.get("function_call", [])) == 0:
            LOG_INFO("=_verify_function_call_in_wenxin_message_response========1======False")
            return False
    ...
    #=======================================================
    def _verify_text_content_in_wenxin_message_response(self,response: dict) -> bool:

        if len(response.get("body", [])) == 0:
            LOG_INFO("=_verify_text_content_in_wenxin_message_response========1======False")
            return False

        if response["body"].get("result", None) is None:
            LOG_INFO("=_verify_text_content_in_wenxin_message_response========2======False")
            return False
    ...
    #=======================================================
    def _verify_null_msg_in_wenxin_stream_response(self,response: dict) -> bool:
    ...
    #   检查 是否是function call的应答 在 qwen的响应消息里面
    #=======================================================
    def _verify_function_call_in_wenxin_stream_message_response(self,response: dict) -> bool:
    ...
    #==========================================================
    def _verify_text_content_in_wenxin_stream_message_response(self,response: dict) -> bool:
