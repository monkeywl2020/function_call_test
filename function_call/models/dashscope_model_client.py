...
#-----------------------------------
class DashscopeChatWarpperClient(ModelClient):

    #千问系列默认api_type是 qwen    
    api_type:str = "dashscope"
    #api_type:str = "dashscope-llama3.1-405b"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        LOG_INFO("DashscopeChatWarpperClient::__init__-----------")
        self.model = model #保存模型
        self.generate_args = generate_args or {}

        #===================================================
        # 导入 dashscope, 本地qwen模型可以使用openAI的接口进行处理
        #===================================================
        try:
            import dashscope
        except ImportError as e:
            raise ImportError(
                "Cannot find dashscope package, please install it by "
        ...
        if 'api_protocol' in kwargs:
            self.api_protocol =  kwargs['api_protocol']  # 目前暂时这么设置，后续可以设置
        else:
        ...
        if 'max_tokens' in kwargs:
            ...
            self.max_tokens = 2000  # 目前暂时这么设置，后续可以设置
    ...
    def format(self, *args: Union[Msg, Sequence[Msg]]) -> List[dict]:
        ...
        #---------------------------------------------------
        for arg in args:
            LOG_INFO("DashscopeChatWarpperClient::format:=============== arg in args:",arg)
            ...
            if isinstance(arg, Msg): # 如果是 Msg
                if arg.url is not None:# 如果有 url
                    ...
                    if arg.role != "tool":
                        ...
                        #---------------------------------------------------
                        if arg.get("message"):
                            LOG_INFO("DashscopeChatWarpperClient::format:=============== message in Msg:",arg.get("message"))
                            messages.append(arg.get("message"))

                        # 下面是工具调用结果的消息
                        elif arg.get("tool_responses"):
                            LOG_INFO("DashscopeChatWarpperClient::format:=============== tool_responses in Msg:",arg.get("tool_responses"))
                            ...
                            messages.append(arg)
                LOG_INFO("DashscopeChatWarpperClient::format:=============== arg to message:",messages)
        ...
        LOG_INFO("DashscopeChatWarpperClient::format:=============== end! final messages:",messages)
        return messages
    ...
    def _generate_response(
        ...
        LOG_INFO("DashscopeChatWarpperClient::_generate_response:---------11-- kwargs",kwargs)
        ...
        if stream:
            DashScopekwargs["stream_options"] = {"include_usage": True}

        # 如果标识使用工具，并且有工具管理器,则获取工具描述
        if use_tools and self.tools_manager:
            tools = self.tools_manager.get_tool_descriptions(type = "dashscope")
            if len(tools) > 0:
                DashScopekwargs["tools"] = tools

        #-------------------------------------------------------------
        # 3： 调用模型接口 同步
        #-------------------------------------------------------------
        LOG_INFO("DashscopeChatWarpperClient::_generate_response:-------cccc---- kwargs:\n",DashScopekwargs)
        response = self.client.Generation.call(**DashScopekwargs)
        LOG_INFO("DashscopeChatWarpperClient::_generate_response:-------cccc---- response:\n",response)
        ...
        if stream:
            ...
            try:
                ...
                LOG_INFO("DashscopeChatWarpperClient::_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("DashscopeChatWarpperClient::_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("DashscopeChatWarpperClient::_generate_response: -------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("DashscopeChatWarpperClient::_generate_response: -------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("DashscopeChatWarpperClient::_generate_response: -------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"DashscopeChatWarpperClient::_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            LOG_INFO("DashscopeChatWarpperClient::_generate_response:-------ccc222---- response:\n",response_output)
            if self._verify_text_content_in_dashscope_message_response(response_output):
                #---------------------------------------------------
                ...
                # return response
                return ModelResponse(
                    text=response_output["choices"][0]["message"]["content"],
                    raw=response,
                )
            # 如果是tools的function call响应。
            elif self._verify_function_call_in_dashscope_message_response(response_output):
                #---------------------------------------------------
    ...
    async def _a_generate_response(
        ...
        LOG_INFO("DashscopeChatWarpperClient::_a_generate_response-----------")
        # 其他参数
        kwargs = {**self.generate_args, **kwargs}
        ...
        # 3： 调用模型接口 异步
        #-------------------------------------------------------------
        LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------cccc---- kwargs:",kwargs)
        response = self.client.chat.completions.create(**kwargs)
        LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------cccc---- response:",response)
        ...
        if stream:
            ...
            try:
                ...
                first_chunk = first_chunk.model_dump()
                firs_role = first_chunk["choices"][0]["delta"]["role"]
                LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            except StopIteration:
                raise RuntimeError("Empty response received from OpenAI API.")
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"DashscopeChatWarpperClient::_a_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            LOG_INFO("DashscopeChatWarpperClient::_a_generate_response:-------cccc222---- response:model_dump():",response)
    ...
    #=======================================================
    def _verify_function_call_in_dashscope_message_response(self,response: dict) -> bool:
    ...
    #=======================================================
    def _verify_text_content_in_dashscope_message_response(self,response: dict) -> bool:
    ...
    #=======================================================
    def _verify_null_msg_in_dashscope_stream_response(self,response: dict) -> bool:
    ...
    #   检查 是否是function call的应答 在 qwen的响应消息里面
    #=======================================================
    def _verify_function_call_in_dashscope_stream_message_response(self,response: dict) -> bool:
    ...
    #==========================================================
    def _verify_text_content_in_dashscope_stream_message_response(self,response: dict) -> bool:
