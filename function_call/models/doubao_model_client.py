...
#-----------------------------------
class DoubaoChatWarpperClient(ModelClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        LOG_INFO("DoubaoChatWarpperClient::__init__-----------")
    ...
    def format(self, *args: Union[Msg, Sequence[Msg]]) -> List[dict]:
        ...
        #---------------------------------------------------
        for arg in args:
            LOG_INFO("DoubaoChatWarpperClient::format:=============== arg in args:",arg)
            ...
            if isinstance(arg, Msg): # 如果是 Msg
                if arg.url is not None:# 如果有 url
                    ...
                    if arg.role != "tool":
                        ...
                        #---------------------------------------------------
                        if arg.get("message"):
                            LOG_INFO("DoubaoChatWarpperClient::format:=============== message in Msg:",arg.get("message"))
                            messages.append(arg.get("message"))

                        # 下面是工具调用结果的消息
                        elif arg.get("tool_responses"):
                            LOG_INFO("DoubaoChatWarpperClient::format:=============== tool_responses in Msg:",arg.get("tool_responses"))
                            ...
                            messages.append(arg)
                LOG_INFO("DoubaoChatWarpperClient::format:=============== arg to message:",messages)
        ...
        LOG_INFO("DoubaoChatWarpperClient::format:=============== end! final messages:",messages)
        return messages
    ...
    def format_function_call_rsp(self, response:dict) -> List[dict]:
        ...
        '''
        {
            "id": "0217255242226723c183a1fb82c613f4d4485b7d388d5b47bf02c",
            "choices": [
                {
                ...
                "logprobs": null,
                "message": {
                    "content": "\n当前提供了 1 个工具，分别是['get_current_weather']，用户需求是查询武汉、北京和深圳的天气，需要调用get_current_weather工具进行查询",
                    "role": "assistant",
                    "function_call": null,
                    "tool_calls": [
                    {
                        "id": "call_n4pjbu6fzkfocpd1y50wx6r2",
                        "function": {
            ...
            "created": 1725524228,
            "model": "doubao-pro-32k-functioncall-240515",
        ...
        LOG_INFO("DoubaoChatWarpperClient::format_function_call_rsp:----------- formatted_response:\n",formatted_response)
    ...
    def _generate_response(
        ...
        LOG_INFO("DoubaoChatWarpperClient::_generate_response:---------11-- kwargs",kwargs)
        ...
        if stream:
            doubaokwargs["stream_options"] = {"include_usage": True}

        # 如果标识使用工具，并且有工具管理器,则获取工具描述
        if use_tools and self.tools_manager:
            tools = self.tools_manager.get_tool_descriptions(type = "openai")
            if len(tools) > 0:
                doubaokwargs["tools"] = tools

        #-------------------------------------------------------------
        # 3： 调用模型接口 同步
        #-------------------------------------------------------------
        LOG_INFO("DoubaoChatWarpperClient::_generate_response:-------cccc---- doubaokwargs:\n",doubaokwargs)
        response = self.client.chat.completions.create(**doubaokwargs)
        LOG_INFO("DoubaoChatWarpperClient::_generate_response:-------cccc---- response:\n",response)
        ...
        if stream:
            ...
            try:
                ...
                LOG_INFO("DoubaoChatWarpperClient::_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("DoubaoChatWarpperClient::_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("DoubaoChatWarpperClient::_generate_response: -------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("DoubaoChatWarpperClient::_generate_response: -------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("DoubaoChatWarpperClient::_generate_response: -------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"DoubaoChatWarpperClient::_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            LOG_INFO("DoubaoChatWarpperClient::_generate_response:-------ccc222---- response:\n",response)
           
            if self._verify_text_content_in_doubao_message_response(response):
                #---------------------------------------------------
            ...
            elif self._verify_function_call_in_doubao_message_response(response):
                #---------------------------------------------------
    ...
    async def _a_generate_response(
        ...
        LOG_INFO("DoubaoChatWarpperClient::_a_generate_response-----------")
        # 其他参数
        kwargs = {**self.generate_args, **kwargs}
        ...
        # 3： 调用模型接口 异步
        #-------------------------------------------------------------
        LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------cccc---- kwargs:",kwargs)
        response = self.client.chat.completions.create(**kwargs)
        LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------cccc---- response:",response)
        ...
        if stream:
            ...
            try:
                ...
                first_chunk = first_chunk.model_dump()
                firs_role = first_chunk["choices"][0]["delta"]["role"]
                LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            except StopIteration:
                raise RuntimeError("Empty response received from OpenAI API.")
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"DoubaoChatWarpperClient::_a_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    if callback is not None:
            ...
            response = response.model_dump()
            LOG_INFO("DoubaoChatWarpperClient::_a_generate_response:-------cccc222---- response:model_dump():",response)
    ...
    #=======================================================
    def _verify_function_call_in_doubao_message_response(self,response: dict) -> bool:
    ...
    #=======================================================
    def _verify_text_content_in_doubao_message_response(self,response: dict) -> bool:
        ...
        # 正常用户消息 finish_reason 不会为 tool_use
    ...
    #=======================================================
    def _verify_null_msg_in_doubao_stream_response(self,response: dict) -> bool:
    ...
    #   检查 是否是function call的应答 在 qwen的响应消息里面
    #=======================================================
    def _verify_function_call_in_doubao_stream_message_response(self,response: dict) -> bool:
    ...
    #==========================================================
    def _verify_text_content_in_doubao_stream_message_response(self,response: dict) -> bool:
